"""
server/environment.py — Core simulation logic for the Data Cleaning environment.

Responsibilities:
  - Generate realistic messy datasets per task (seeded, reproducible)
  - Maintain gold-standard cleaned datasets for grading
  - Run agent code in a restricted sandbox
  - Score output DataFrames with a decomposed reward function
"""
...
from models import (
    DataCleaningAction as Action,
    DataCleaningObservation as Observation,
    DataCleaningState as State,
    ActionType,
    TaskID,
)

# Internal reward dataclass — not part of the public OpenEnv API
...
def _make_ecommerce(rng: random.Random, np_rng: np.random.Generator) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # 1. order_date → string with mixed formats
    ...
    # 4. revenue with mixed currency symbols
    ...
    # 6. status casing + "complete" alias
    ...
    # Recompute gold revenue after potential quantity clip (gold stays clean)
...
def _make_financial_audit(rng: random.Random, np_rng: np.random.Generator) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # ---- Now introduce controlled corruptions ----
...
# DataFrame ↔ parquet serialisation
